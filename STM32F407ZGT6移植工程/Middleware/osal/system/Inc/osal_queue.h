 ...
 *
 * 等待语义：
 * - timeout_ms = 0U：不等待，立即返回。
 * - timeout_ms = N ：最多等待 N 毫秒。
 * - timeout_ms = OSAL_WAIT_FOREVER：永久等待。
 */
 ...
 * @param q 队列句柄。
 * @param item 源消息指针。
 * @param timeout_ms 等待时间，单位为毫秒；支持 0U / N / OSAL_WAIT_FOREVER。
 * @return OSAL 状态码。
 * @note 如果当前任务进入等待，OSAL 会把它真正置为 BLOCKED，并在队列可写或超时后再唤醒。
 * @note 在当前这种协作式任务模型里，推荐写法是“只有返回 OSAL_OK 才继续处理”，
 *       否则本轮任务函数直接 return，等待调度器下次再次进入。
 */
 ...
 * @param timeout_ms 等待时间，单位为毫秒；支持 0U / N / OSAL_WAIT_FOREVER。
 * @return OSAL 状态码。
 * @note 如果当前任务进入等待，OSAL 会把它真正置为 BLOCKED，并在队列收到消息或超时后再唤醒。
 * @note 在当前这种协作式任务模型里，推荐写法是“只有返回 OSAL_OK 才继续处理”，
 *       否则本轮任务函数直接 return，等待调度器下次再次进入。
 */
