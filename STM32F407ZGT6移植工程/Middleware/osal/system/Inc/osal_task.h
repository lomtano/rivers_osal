...
/**
 * @brief 任务当前的阻塞等待原因。
 * @note 该枚举主要用于 OSAL 内部管理不同阻塞对象，也方便调试时判断
 *       任务当前是在普通休眠、等队列可写还是等队列可读。
 */
typedef enum {
    OSAL_TASK_WAIT_NONE = 0,
    OSAL_TASK_WAIT_SLEEP,
    OSAL_TASK_WAIT_QUEUE_SEND,
    OSAL_TASK_WAIT_QUEUE_RECV
