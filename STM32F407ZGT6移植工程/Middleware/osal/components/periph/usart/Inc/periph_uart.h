...
/* 桥接层只要求底层能可靠发送“单个字节”，其余字符串/数组发送都由组件层循环完成。 */
typedef struct {
    /* context 由 create() 时传入，通常是底层 UART 句柄。 */
 ...
 * @note 组件层不会接管 context 的所有权，context 的生命周期仍由调用方保证。
 */
 ...
 * @param uart USART 组件句柄。
 * @return OSAL 状态码。
 * @note 只是登记“当前控制台走哪个 USART”，不会复制对象也不会转移所有权。
 */
