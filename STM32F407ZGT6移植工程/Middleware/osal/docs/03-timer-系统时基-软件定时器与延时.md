...
- `osal_timer_create()`
- `osal_timer_start()`
- `osal_timer_set_period()`
- `osal_timer_set_remaining()`
- `osal_timer_stop()`
- `osal_timer_delete()`
...
`period_us` 是周期定时器的周期，也是单次定时器重新启动时使用的超时时长。
`expiry_us` 是下一次绝对到期时间。

当前软件定时器内部没有独立硬件计数器；所谓“当前计数值”，在这个实现里等价于“距离下一次到期还剩多少时间”。
...
软件定时器支持在程序运行过程中动态调整周期和当前剩余计数：

- `osal_timer_set_period(timer_id, period_us)`
  修改周期或单次超时时长。如果定时器正在运行，下一次到期时间会从当前时刻重新计算为 `now + period_us`；如果定时器已停止，只修改保存的周期值，不会自动启动。
- `osal_timer_set_remaining(timer_id, remaining_us)`
  修改正在运行定时器距离下一次到期的剩余时间。如果传入 `0`，下一轮 `osal_timer_poll()` 会尽快处理该定时器。停止状态下没有当前计数值，因此该接口会返回失败。

这两个接口仍然只允许任务态调用，不允许在 ISR 中调用。
