...
/*
 * 说明：
 * 1. 本文件是根据 osal_platform_cortexm 模板填写出的 STM32F4 板级适配层。
 * 2. 当前目标板示例是 STM32F407ZGT6。
 * 3. OSAL 和 MCU SDK 的直接耦合点，集中放在这个文件和对应的 .c 文件里。
 * 4. system 层不关心 HAL、寄存器名字和具体扇区布局，这些差异都在这里处理。
 */

/**
 * @brief Flash 演示默认使用的内部地址。
 * @note 启用 Flash 示例前，请先改成你项目中明确预留的用户扇区。
 */
 ...
 * =========================
 * 这里通过宏把 OSAL 和 STM32 HAL / Cortex-M SysTick 接起来。
 * 注意这里只提供“原始数据”：
 * - 当前 SysTick 输入时钟频率
 * - 重装值寄存器
 * - 当前计数值寄存器
